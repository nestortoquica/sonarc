...
let HeaderComponent = class HeaderComponent {
    ...
    ngOnInit() {
        ...
        if (this.currentURL.indexOf("/pay-balance") != -1) {
            this.translate.get('payments.balance').subscribe((text) => {
                this.tab = text;
            });
        }
        else if (this.currentURL.indexOf("/notifications") != -1) {
            this.translate.get('general.notifications').subscribe((text) => {
                this.tab = text;
            });
        }
        else if (this.currentURL.indexOf("/profile") != -1) {
            this.translate.get('general.profile').subscribe((text) => {
                this.tab = text;
            });
        }
        else if (this.currentURL.indexOf("/pay") != -1) {
            this.translate.get('general.Entregas').subscribe((text1) => {
                this.translate.get('general.payment').subscribe((text2) => {
    ...
    navigate(profile) {
        if (profile.submenu_name == 'Log Out') {
            ...
            this._router.navigateByUrl(profile.route_sub_menu);
    }
};

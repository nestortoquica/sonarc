...
export class DashboardComponent implements OnInit {
  ...
  onCheckedPackChanged(value:boolean){
...
export class DialogContentAddAddress {
  nameForTheDirecction;
  address_locker;
  invitationCode;
  status;
  ...
  onAddDirection(){
    ...
    if(this.editData){
      ...
      if(this.disableBtn){
        ...
        this.ServerUserService.post_add_locker_user(data).subscribe( res => {   // Cuando:: require_invitation == 0 (no requiere codigo de invitacion)
          if( res.mensaje_return.ERROR == false ){
            localStorage.setItem("add_address", 'success');
...
export class DialogContentDeleteAddress {
